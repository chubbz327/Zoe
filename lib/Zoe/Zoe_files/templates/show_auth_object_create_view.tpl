...
<h1>Create New #__OBJECTSHORTNAME__</h1>
%my $url = url_for('#__CREATEURL__', id=>$object->get_primary_key_value);
<form method='POST' action='<%= $url %>' id='form_<%=$object->get_primary_key_value %>' enctype="multipart/form-data">
<div id ='stored_inputs' style="display: none;">
...
<p>
		...
		<em>*</em>
		<input  type='password'  class='required' required  
...
% my %linked_create = %{$object->get_linked_create()};
	...
	<div>
		<button type="button" class="btn btn-link"
				...
				onclick="toggle_add('#new_<%=$linked_object_short_name%>', '<%=$linked_object_short_name%>', '#new_<%=$linked_object_short_name%>_button' )">
		New <%= $linked_object_short_name %> </button>
	</div>
	<div id='previously_added_<%=$linked_object_short_name%>'>
	</div>
	<div class='well' id="new_<%=$linked_object_short_name%>" style='display:none;'>
		...
		onclick="add_to_request_param('<%=$member_name%>', 'ADD_<%= $linked_object_short_name%>_', '<%=$linked_object_short_name%>', '<%= $to_string_name%>' )"
		>Add <%= $linked_object_short_name %></button>
	</div>
	%}
	%}
<br/>

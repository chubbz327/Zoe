...
function add_to_request_param(request_param, prefix, object_name,
		to_string_name, hide_add_button, type_prefix) {
	...
	$(div_id + " :input").each(
			function() {
				if (this.validity.valid === true) {
					...
					console.log('to_string_name ' + to_string_name  + ' member_name ' + member_name + " " + type_prefix + to_string_name );
					
					if (to_string_name != '0') {
						if ( (member_name == to_string_name) || ( member_name == type_prefix +  to_string_name) ) {
...
function submit_form(formid, callback) {
	 $.ajax({
	        type: "POST",
